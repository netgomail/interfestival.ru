	...
	foreach($arResult["MONTH"] as $week => $arWeek)
	{
		foreach($arWeek as $day => $arDay)
		{
			if(count($arDay["events"])>0)
			{
				...
				$LIST_URL .= URLEncode($arParams["FILTER_NAME"]."[>="."PROPERTY_START_DATE_VALUE")."=".URLEncode($strFROM);
				$LIST_URL .= "&".URLEncode($arParams["FILTER_NAME"]."[<"."PROPERTY_START_DATE_VALUE")."=".URLEncode($strTO);
