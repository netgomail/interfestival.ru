...
<? 
	if($date_now >= $fdate_now){
		...
		if(in_array($arFests['ID'], $arFestActiveId)){
			continue;
		}
		$arFestActiveId[$arFests['ID']] = $arFests['ID'];	
		$date_passed = false;
}?>
...
<? if($_REQUEST['festFilter_pf']['MONTH'] == ""): // если применен фильтр по месяцу не выводим прошедшие фестивали?>  
...
<? if($date_now >= $fdate_now){
					$date_passed = true;
					 ...
					 if(in_array($arFests['ID'], $arFestPastId)){
						 	continue;
						}
						$arFestPastId[$arFests['ID']] = $arFests['ID'];
			} else {
					$date_passed = false;
					continue;
    }?>
