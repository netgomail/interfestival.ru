...
if($arParams["TYPE"] || !in_array($arParams["DATE_FIELD"], array("DATE_ACTIVE_FROM", "DATE_ACTIVE_TO", "TIMESTAMP_X", "DATE_CREATE", "PROPERTY_FEST_DATES_START")))
...
if($this->StartResultCache(false, array($currentMonth, $currentYear, $todayYear, $todayMonth, $todayDay)))
{
    ...
    $arFilter = Array(
		...
		$property_start_date_begin = date("Y-m-d", mktime(0,0,0,$currentMonth,$curr,$currentYear));
    $property_start_date_end = date("Y-m-d", mktime(0,0,0,$currentMonth+1,1,$currentYear));
    
		$arFilter[] = Array(
        ">=" . "PROPERTY_START_DATE" => $property_start_date_begin,
        "<" . "PROPERTY_START_DATE" => $property_start_date_end
    );

	$arSelectedFields = Array("ACTIVE", "PROPERTY_START_DATE", $arParams["DATE_FIELD"], "ID", "IBLOCK_ID", "SITE_ID", "DETAIL_PAGE_URL", "NAME", "LANG_DIR", "SORT", "IBLOCK_TYPE", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE","PREVIEW_PICTURE");
	...
	//	file_put_contents($_SERVER["DOCUMENT_ROOT"].'/log2.txt', print_r($arItem, true));
		
		
		
		$arItem['PREVIEW_PICTURE'] = CFile::GetFileArray($arItem['PREVIEW_PICTURE']);
		$arDays[ConvertDateTime($arItem["PROPERTY_START_DATE_VALUE"], CLang::GetDateFormat("SHORT"))][] = $arItem;
	}
	...
	for ($i = 0; $i < 6; $i++)
	{
		...
		for ($j = 0; $j < 7; $j++)
		{
            ...
            elseif ($d == $todayDay && $m == $todayMonth && $y == $todayYear && !$bBreak)
			{
			...
			if(is_set($arDays[$tmpDate]))
			{
				...
				foreach($arDays[$tmpDate] as $dayNews)
				{
					...
					$arDay["events"][] = array(
						...
						"title"=>$title,
						"preview"=>$sTitle,
						"preview_picture"=>$dayNews["PREVIEW_PICTURE"],
                        "preview_text"=>$dayNews["PREVIEW_TEXT"],
						"id_event"=>$dayNews["ID"],
